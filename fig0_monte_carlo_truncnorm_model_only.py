...
# Percentile interval to plot. 68.27 is approximately ±1 sigma for a normal
# distribution. Use 95.0 for an approximate 95% interval.
MC_INTERVAL_PERCENT = 68.27
...
def run_monte_carlo_for_series(model, series_name, spec, data, rng):
    ...
    low_q = 50.0 - MC_INTERVAL_PERCENT / 2.0
    high_q = 50.0 + MC_INTERVAL_PERCENT / 2.0
    ...
    for col in TRANSFORMED_COLS:
        ...
        region[f"{col}_lo"] = np.nanpercentile(pivot.to_numpy(), low_q, axis=0)
        region[f"{col}_median"] = np.nanpercentile(pivot.to_numpy(), 50.0, axis=0)
        region[f"{col}_hi"] = np.nanpercentile(pivot.to_numpy(), high_q, axis=0)
    ...
    settings = {
        ...
        "mc_interval_percent": MC_INTERVAL_PERCENT,
...
def plot_model_and_data(ax, series, central_df, mc_df, region, data_df,
    ...
    draw_parametric_region(
        ...
        label=SERIES_REGION_LABELS.get(
            series,
            f"{display_label(series)} {MC_INTERVAL_PERCENT:g}% MC region",
        ),
        alpha=REGION_ALPHA,
        color=region_color,
    )
